...
SparseMatrix SparseMatrix::operator*(SparseMatrix &other)
{
    ...
    cout<<other.col_num * r<<endl;
    cout<<row_num * other.r<<"row_num * other.r"<<endl;
    //method 1
    
    for (int i=0;i<r;i++){
        for (int j=0;j<other.r;j++){
            if (vec[i].col==other.vec[j].row){
                int mul_row=vec[i].row;
                int mul_col=other.vec[j].col;
                mul_SM.M[mul_row][mul_col]+=vec[i].val*other.vec[j].val;
            }
        }
    }
    //method 2
